...
# All rights reserved!
#-------------------------------------------------
package Diamond::Lib::Tk::TimePick2;
#-------------------------------------------------
use strict;
use warnings;
use Tk::Frame;
...
#-------------------------------------------------
@Diamond::Lib::Tk::TimePick2::ISA = qw(Tk::Frame);
$Diamond::Lib::Tk::TimePick2::VERSION = '0.02';
Construct Tk::Widget "Diamond::Lib::Tk::TimePick2";
#-------------------------------------------------

sub ReloadTime {
  # print Dumper($_[0]->{_timestring});
  # parse this time into the current time structure
  if ($_[0]->{_timestring} =~ /^(\d\d):(\d\d):(\d\d)$/) {
    # print "<$1><$2><$3>\n";
    $_[0]->{_hours} = $1;
    $_[0]->{_minutes} = $2;
    $_[0]->{_seconds} = $3;
	...
	($_ eq 's')	&& do
	  {
	    ...
	    $tp->{_timestring} .= sprintf("%02i",$tp->{_seconds});
	    last(SWITCH); 
	  };
	($_ eq 'm')	&& do
	  {
	    ...
	    $tp->{_timestring} .= sprintf("%02i",$tp->{_minutes});
	    last(SWITCH); 
	  };
	($_ eq 'h')	&& do
	  {
	    ...
	    $tp->{_timestring} .= sprintf("%02i",$tp->{_hours});
	    last(SWITCH); 
	  };
      }
    }
...
sub SetTime
  {
	...
	($_ eq 'h')	&& do
	  {
	    $self->{_hours}		= $t[$index];
	    $self->{_timestring}	.= sprintf("%02i",$t[$index]);
	    last(SWITCH);
	  };
	($_ eq 'm')	&& do
	  {
...
sub GetTime
  {
	...
	($_ eq 'h')	&& do
	  {
	    $self->{_timestring}	.= sprintf("%02i",$self->{_hours});
	    last(SWITCH);
	  };
	($_ eq 'm')	&& do
	  {
	    $self->{_timestring}	.= sprintf("%02i",$self->{_minutes});
	    last(SWITCH);
	  };
	($_ eq 's')	&& do
	  {
	    $self->{_timestring}	.= sprintf("%02i",$self->{_seconds});
	    last(SWITCH);
	  };
      }
    }
...
Diamond::Lib::Tk::TimePick2 - Perl extension for a graphical user interface to pick timestrings syntax mistake-secure
